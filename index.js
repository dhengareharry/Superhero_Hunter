const Main = (function () {
        ...
        searchResults.forEach((element) => {
            ...
            li.innerHTML = `
                ...
                <div class="search-right">
                    ...
                    <button class="btn add-to-fav" data-id=${element.id} style="display: ${indexOfSuperHeroInFavourites === -1 ? 'block' : 'none'}">Add to favorites</button>
                    <button class="btn remove-from-fav" data-id=${element.id} style="display: ${indexOfSuperHeroInFavourites === -1 ? 'none' : 'block'}">Remove from favorites</button>
                </div>
            `;
            searchList.appendChild(li);
        });
    }
 /* Removing  All Search Results from the UI */
 ...
 // Show loader and remove existing search results
    ...
    function init() {
        searchInput.addEventListener('keyup', Common.debounce(handleSearch, 500));
